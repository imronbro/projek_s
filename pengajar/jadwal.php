...
// Ambil mentor_id berdasarkan email
$query = "SELECT pengajar_id FROM mentor WHERE email = ?";
$stmt_mentor = $conn->prepare($query);
$stmt_mentor->bind_param("s", $user_email);
$stmt_mentor->execute();
$result_mentor = $stmt_mentor->get_result();
if ($row_mentor = $result_mentor->fetch_assoc()) {
    $pengajar_id = $row_mentor['pengajar_id'];
} else {
